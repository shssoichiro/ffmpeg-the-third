...
fn main() -> Result<(), ffmpeg::Error> {
    ...
    let mut context = input(env::args().nth(1).expect("missing file")).build()?;

    for (k, v) in context.metadata().iter() {
        println!("{k}: {v}");
    }

    if let Some(stream) = context.best_stream().find(media::Type::Video) {
        println!("Best video stream index: {}", stream.index());
    }

    if let Some(stream) = context.best_stream().find(media::Type::Audio) {
        println!("Best audio stream index: {}", stream.index());
    }

    if let Some(stream) = context.best_stream().find(media::Type::Subtitle) {
    ...
    for stream in context.streams() {
        ...
        } else if codec.medium() == media::Type::Audio {
